...
fn compute_channel_histogram(data: &[f32], _bitdepth_max: f32, with_markers: bool) -> ChannelHistData {
    let (data_min, data_max) = data_min_max(data);
    let range = data_max - data_min;

    // Build parallel histogram
    ...
    // Replicate autostretch marker logic
    const HIGH_PCTILE: f64 = 0.9998;
    const K_MIDTONE: f32 = 1.5;
    ...
    // Sky background mode in lower 33%
    ...
    // Noise sigma from left-side half-max of mode peak
    let sigma = {
        let half_count = bins[mode_bin] / 2;
        let left_half_bin = (0..mode_bin)
            .rev()
            .find(|&i| bins[i] <= half_count)
            .unwrap_or(0);
        let sigma_bins = (mode_bin - left_half_bin) as f32 / 0.8326_f32;
        (sigma_bins * bin_width).max(bin_width)
    };
    ...
    // Express as fractions of [data_min, data_max]
...
pub fn compute_histogram(img: &FitsImage, with_markers: bool) -> HistogramData {
    ...
    let bd = img.bitdepth_max;

    let channels = if img.channels == 3 {
        // Run R, G, B in parallel
        let r = &img.data[0..npix];
        let g = &img.data[npix..2 * npix];
        let b = &img.data[2 * npix..3 * npix];
        std::thread::scope(|s| {
            let rh = s.spawn(|| compute_channel_histogram(r, bd, with_markers));
            let gh = s.spawn(|| compute_channel_histogram(g, bd, with_markers));
            let bh = s.spawn(|| compute_channel_histogram(b, bd, with_markers));
            vec![rh.join().unwrap(), gh.join().unwrap(), bh.join().unwrap()]
        })
    } else {
        let plane = &img.data[..npix.min(img.data.len())];
        vec![compute_channel_histogram(plane, bd, with_markers)]
    };
