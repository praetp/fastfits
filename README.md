...
- **Supernova overlay** — press `V` or click **SNe** to overlay live supernova annotations; queries both [SIMBAD](https://simbad.u-strasbg.fr/) (CDS, Strasbourg) and the [Transient Name Server](https://www.wis-tns.org/) in parallel; TNS covers classified SNe within days of announcement; no API key required; results are merged and labelled (e.g. "SN 2023ixf"); cached per field+month for older images; a spinner shows while fetching; enabled by default; requires WCS headers
...
- **FITS header inspector** — left panel shows all header key/value pairs alphabetically; numeric values are displayed in plain decimal notation by default (toggle with the **1.23 / 1E0** button in the panel header); raw values are always used when copying; when `DATE-OBS` is present, the panel also shows a **lunar phase indicator** (emoji, phase name, illumination %, and age in days) derived from the observation timestamp
